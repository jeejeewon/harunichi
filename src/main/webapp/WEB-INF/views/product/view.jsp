...
<div class="product-detail-container">
    ...
    <div class="writer-info">
        <c:choose>
            <c:when test="${not empty product.writerProfileImg}">
                <img src="${ctx}/images/profile/${product.writerProfileImg}" class="writer-img" alt="프로필" />
            </c:when>
            <c:otherwise>
        ...
        <div class="product-info-box">
            ...
            <div class="action-buttons">
                ...
                <button id="likeButton" class="btn-like">
                    <i id="likeIcon" class="fa-regular fa-heart">❤️</i> <span id="likeCount">0</span>
                </button>
            </div>
        </div>
    </div>
	...
	<c:if test="${sessionScope.id eq product.productWriterId or sessionScope.id eq 'admin'}">
	    <div class="detail-buttons" style="text-align: left;">
	        ...
	        <form id="deleteForm" action="${ctx}/product/delete" method="post" style="display: inline;">
	            <input type="hidden" name="productId" value="${product.productId}">
	            <button type="submit" class="btn btn-delete" onclick="return checkLogin()">삭제</button>
	        </form>
	        <a href="${ctx}/product/list" class="btn btn-list" style="display: inline-block;">목록</a>
	    </div>
	</c:if>
	
	<!-- 로그인은 했지만 작성자가 아닌 사용자 OR 로그인하지 않은 사용자 -->
	<c:if test="${sessionScope.id ne product.productWriterId and sessionScope.id ne 'admin'}">
	    <div class="detail-buttons" style="text-align: left;">
